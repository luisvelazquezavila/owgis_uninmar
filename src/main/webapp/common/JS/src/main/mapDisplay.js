...
function initMenus() {
    ...
    if (netcdf) {
        //load the palettes
        console.log("OWGIS DEBUG - antes de loadPalettes");

        owgis.ncwms.palettes.loadPalettes();

        console.log("OWGIS DEBUG - después de loadPalettes");
    ...
    // Inicializar calendario para cualquier capa que tenga información temporal.
    // Las fechas ya fueron obtenidas desde Java/ERDDAP y están en layerDetails.datesWithData.
    
    console.log("OWGIS TIME DEBUG - netcdf:", netcdf);
    console.log("OWGIS TIME DEBUG - isTimeEnabled:", isTimeEnabled);
    console.log("OWGIS TIME DEBUG - layerDetails:", layerDetails);
    
    if (isTimeEnabled) {
        console.log("OWGIS TIME - Inicializando calendario temporal");
        initCalendars();
    }
