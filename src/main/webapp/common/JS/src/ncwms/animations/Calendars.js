...
function initCalendars() {
    ...
    console.log("========== INIT CALENDARS NUEVO ==========");
    ...
    if (typeof datesWithData !== "undefined") {
        ...
        console.log("OWGIS CALENDAR DEBUG - reqTIME:", reqTIME);

        var timesWithData = layerDetails.timesWithData || {};
        var hoursForFirstDay = [];

        if (timesWithData[reqTIME]) {
            hoursForFirstDay.push(timesWithData[reqTIME]);
        }

        console.log(
                "OWGIS CALENDAR DEBUG - hoursForFirstDay:",
                hoursForFirstDay
                );

        console.log(
                "OWGIS CALENDAR DEBUG - hoursForFirstDay.length:",
                hoursForFirstDay.length
                );

        console.log("==========================================");

        //We verify that we have more than one day
        if (minValidDate < maxValidDate) { //We have more than one day
            ...
            $("#cal-start").datepicker("setDate", minValidDate);
            $("#cal-end").datepicker("setDate", maxValidDate);

            console.log(
                "OWGIS TIME DEBUG - fecha start establecida:",
                $("#cal-start").val()
            );

            console.log(
                "OWGIS TIME DEBUG - fecha end establecida:",
                $("#cal-end").val()
            );
...
function updateCalendarOpts(calUpdated, forcedDate) {
    
    var startDateTxt = forcedDate || $("#cal-start").val();
    ...
    if (calInitialized) {
        ...
        console.log(
            "OWGIS TIME DEBUG - startDateTxt:",
            startDateTxt
        );

        console.log(
            "OWGIS TIME DEBUG - endDateTxt:",
            endDateTxt
        );

        // Si los calendarios todavía no tienen fecha,
        // no continuamos.
        if (!startDateTxt || !endDateTxt) {

            console.log(
                "OWGIS TIME DEBUG - Los calendarios todavía no tienen fecha."
            );
        ...
        if (calUpdated === owgis.constants.startcal) {
            ...
            if (Object.keys(timesWithData).length > 0) {

                console.log(
                    "OWGIS TIME - Usando timesWithData de ERDDAP"
                );
            ...
            var timesWithData =
                layerDetails.timesWithData || {};

            if (Object.keys(timesWithData).length > 0) {

                console.log(
                    "OWGIS TIME - Usando timesWithData de ERDDAP para END"
                );
